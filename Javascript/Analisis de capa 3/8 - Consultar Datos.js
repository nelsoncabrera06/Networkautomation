...
function main() 
{
	...
	f = fso.OpenTextFile("DATOS/Modelos equipos.txt", ForReading);
	...
	EQUIPO = crt.Dialog.Prompt("Ingrese el equipo ", "Buscar modelo", "XXXYMU", 0);
	 
	var str;
	while ( f.AtEndOfStream != true )
	{
		EncontrarCoincidencia(f);
		//ingresarAlEquipo(f);
		//tirarComandos();
		//salirdelequipo();
	}
	
	if (aparecio != true) { //  no encontró el equipo
		crt.Dialog.MessageBox("ERROR -  No se encontro el equipo ingresado: " + EQUIPO);	
	}
...
function EncontrarCoincidencia(f)
{	
	var fila, readline, encontrado;
	var lngPosinicial = 0;
	/*
	Columna 1  Col 2	Col 3	Col 4		Col 5
	("EQUIPO", "IP", "COMANDO", "VARIABLE", "DATO");
	*/
	...
	if (lngPos!=-1) { //  encontré una coma ,
		...
		obtenido = linealeida.match(re);
		if (obtenido != null) {
			encontrado = linealeida;
			for(var j=0;j<4;j++){ 
				lngPosfinal = encontrado.indexOf(",");
				encontrado = encontrado.substring(lngPosfinal+1, encontrado.length);
			}
			
			crt.Dialog.MessageBox(encontrado); // si coindice con la RegExp muestra la variable
			aparecio = true;
		}
