...
'Sub Main
  ...
  index = 0
	...
	PUERTO = InputBox("Ingrese la interface futura","CONFIGURACION FUTURA")
		...
		Select Case ESTADO
			Case 0 'recien arranca el script
				...
				if TRANSPORTE = "NO TIENE" then
					...
					codigo = String(4-len(CVLAN), "0") & CVLAN
					'Subinterface_completa = PUERTO & "." & SVLAN & CVLAN
					Subinterface_completa = PUERTO & "." & SVLAN & codigo
					'MsgBox Subinterface_completa
				end if
...
Function funcion_verificar(fileoutput) 'Modelo de ejemplo HUAWEI CX600-X8 - SPE2MU
	...
	if policy_input <> "NO TIENE" then fileoutput.Write "verificar	" & "disp curr configuration qos-profile " & policy_input & vbCrLf
	if policy_output <> "NO TIENE" then fileoutput.Write "verificar	" & "disp curr configuration qos-profile " & policy_output & vbCrLf
	...
	if BGP <> "NO TIENE" then 
		if VRF <> "NO TIENE" then
			fileoutput.Write "verificar	" &  "disp curr conf bgp | begin " & VRF & vbCrLf
		else
			fileoutput.Write "verificar	" &  "disp curr conf bgp | begin " & NEXTHOP & vbCrLf
		end if
	end if
...
Function funcion_configSUBINTERFACE(fileoutput) 'Modelo de ejemplo HUAWEI CX600-X8A-16A - LOP4MU
	...
	'<MUS4MU>disp curr int Eth-Trunk1.15000002 
	'#
	'interface Eth-Trunk1.15000002
	'description INTEGRA - ARGANARAZ JORGE ALFREDO - REF: 918697 - LINEA: 2866355 - ACC: 5RTRA01/07/GE_5
	'ip address 181.88.155.93 255.255.255.252
	'statistic enable
	'encapsulation qinq-termination
	'qinq termination pe-vid 1500 ce-vid 2
	'arp broadcast enable
	'trust upstream principal
	'trust 8021p
	'qos-profile BEST-EFFORT-50M inbound pe-vid 1500 ce-vid 2 identifier none 
	'qos-profile BEST-EFFORT-50M outbound pe-vid 1500 ce-vid 2 identifier none 
	'#
	'return
	
	'<SSJ4MU>disp curr int Eth-Trunk3.12053818
	'#
	'interface Eth-Trunk3.12053818
	' bandwidth 64
	' description VPN Head Quarter - BANCO MACRO - Ref:309299 Lin:2738315 Acc:ACO11D 15/10
	' ip binding vpn-instance vpn-macro-xot
	' ip address 186.108.136.125 255.255.255.252
	' encapsulation qinq-termination
	' qinq termination pe-vid 1205 ce-vid 3818
	' arp broadcast enable
	' qos-profile BEST-EFFORT-64K inbound pe-vid 1205 ce-vid 3818 identifier none 
	' qos-profile BEST-EFFORT-64K outbound pe-vid 1205 ce-vid 3818 identifier none 
	'#
	'return
	...
	if TRANSPORTE = "NO TIENE" then
		...
		if Policy_input <> "NO TIENE" then fileoutput.Write "Subint	" & "qos-profile " & Policy_input & " inbound vlan " & CVLAN & " identifier none " & vbCrLf   
		if Policy_output <> "NO TIENE" then fileoutput.Write "Subint	" & "qos-profile " & Policy_output & " outbound vlan " & CVLAN & " identifier none "  & vbCrLf                                                                                                        
		fileoutput.Write "Subint	" & "#" & vbCrLf
		'fileoutput.Write "Subint	" & "#---------------------------------------" & vbCrLf
		...
		fileoutput.Write "Subint	" &  "ip address " & IpWAN & " " & Mascara_WAN & vbCrLf
		fileoutput.Write "Subint	" & "statistic enable" & vbCrLf 
		fileoutput.Write "Subint	" & "encapsulation qinq-termination" & vbCrLf
		fileoutput.Write "Subint	" & "qinq termination pe-vid " & SVLAN & " ce-vid " & CVLAN & vbCrLf
		fileoutput.Write "Subint	" & "arp broadcast enable " & vbCrLf
		fileoutput.Write "Subint	" & "trust upstream principal" & vbCrLf
		fileoutput.Write "Subint	" & "trust 8021p " & vbCrLf
		if Policy_input <> "NO TIENE" then fileoutput.Write "Subint	" & "qos-profile " & Policy_input & " inbound pe-vid " & SVLAN & " ce-vid " & CVLAN & " identifier none " & vbCrLf   
		if Policy_output <> "NO TIENE" then fileoutput.Write "Subint	" & "qos-profile " & Policy_output & " outbound pe-vid " & SVLAN & " ce-vid " & CVLAN & " identifier none "  & vbCrLf                                                                                                        
		fileoutput.Write "Subint	" & "#" & vbCrLf
		'fileoutput.Write "Subint	" & "#---------------------------------------" & vbCrLf
	end if
