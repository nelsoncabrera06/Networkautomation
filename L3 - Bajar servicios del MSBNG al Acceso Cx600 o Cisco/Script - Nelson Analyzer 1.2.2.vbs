...
NombreArchivo_Salida = "OUTPUT - Config Puertos.txt"
...
Function funcion_HUAWEI(file, filedatos, index, MARCA, EQUIPO, ESTADO, fileoutTabladeDatos)
	...
	do while Salir <> 2	' mientras no aparezca # hacer
		...
		Salir = crt.screen.WaitForStrings (chr(13), Equipo_gestor, "Line protocol", "Description:", "seconds input rate ", "seconds output rate ") 'esperar por los siguiente caracteres
		Select Case Salir
	...
	Salir = crt.screen.WaitForStrings (chr(13), "#", "as-number ", "description ", "import-route ", Equipo_gestor)
	do while Salir <> 2
		screenrow = crt.screen.CurrentRow
		readline = crt.Screen.Get(screenrow, 1, screenrow, 200)
		readline = rtrim(readline)
	...
	Salir = crt.screen.WaitForStrings (chr(13), "#", NEXTHOP, Equipo_gestor)
	do while Salir <> 2
		screenrow = crt.screen.CurrentRow
		readline = crt.Screen.Get(screenrow, 1, screenrow, 200)
		readline = rtrim(readline)
		...
		lngPos = Instr(readline, NEXTHOP)
		if lngPos = 0 then ' si no encuentro el NEXTHOP en la linea leida
			Salir = 2
		else
			file.Write EQUIPO & " " & SUBINTERFACE & "	" & readline & vbCrLf
		end if 
	...
	Trafico_output = 0
			...
			do while Salir <> 2
				...
				Case 4 ' policy input
					...
					screenrow = crt.screen.CurrentRow
					crt.screen.WaitForString chr(13)
					policy_input = crt.Screen.Get(screenrow, columnaActual, screenrow, 200)
					policy_input = trim(policy_input)
					'MsgBox policy_input	
				Case 5 ' policy output
					...
					screenrow = crt.screen.CurrentRow
					crt.screen.WaitForString chr(13)
					policy_output = crt.Screen.Get(screenrow, columnaActual, screenrow, 200)
					policy_output = trim(policy_output)
				...
				Case 6 ' vrf
					...
					screenrow = crt.screen.CurrentRow
					crt.screen.WaitForString chr(13)
					VRF = crt.Screen.Get(screenrow, columnaActual, screenrow, 200)
					VRF = trim(VRF)
					'MsgBox VRF
				Case 7 ' ipv4 address
					columnaActual = crt.screen.CurrentColumn
					...
					Case 3 ' remote-as 
						...
						Set re = New RegExp
						 ...
						 If re.Test( strid ) Then
							AS_actual = re.Replace(Strid, "$1")
							'MsgBox "AS_actual re hizo match " &  AS_actual
						 Else
							'MsgBox "AS_actual re NO hizo NINGUN match"
						 End If
						Set re = Nothing
	...
	if ip_WAN <> "NO TIENE" then
		...
		'	Set re = Nothing
			...
			do while Salir <> 2
				...
				StringsArray = array(chr(13), "#", Subint_recortada, "arg")
				Salir = encontrarString(readline, StringsArray)
				
				'   192.168.52.16/29 TenGigE0/2/1/0.1310180002 192.168.52.4
					
				Select Case Salir
					Case 3 ' SUBINTERFACE ' Subint_recortada
				'		MsgBox readline
				'		columnaActual = crt.screen.CurrentColumn 
				'		screenrow = crt.screen.CurrentRow
				'		crt.screen.WaitForString chr(13)
